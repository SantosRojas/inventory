...
export const ModelDistributionCharts: React.FC<ModelDistributionChartsProps> = ({ data, summaryData }) => {
  ...
  const { paginatedData, totalPages, filteredCount, total } = useMemo(() => {
    ...
    // Usar datos directos del summary
    ...
    const paginatedData = sortedModels.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  ...
  }, [data?.models, summaryData?.totalPumps, searchTerm, currentPage, sortBy, sortOrder, ITEMS_PER_PAGE]);
      ...
      <div className="mb-4 space-y-3">
        ...
        <div className={`flex gap-2 ${isMobile ? 'flex-col' : 'flex-row'}`}>
          <div className="flex gap-2 flex-1">
            ...
            >
              {sortBy === 'count' && (sortOrder === 'desc' ? <SortDesc className="h-3 w-3" /> : <SortAsc className="h-3 w-3" />)}
              Cantidad
            </button>
            <button
      ...
      {searchTerm && (
        <div className="mb-4 p-3 bg-blue-50 rounded-lg">
          <p className="text-sm text-blue-700">
            Mostrando {filteredCount} de {data.models.length} modelos que coinciden con "{searchTerm}"
          </p>
        </div>
      )}
      ...
      {totalPages > 1 && (
        <div className={`mt-4 space-y-3 ${isMobile ? '' : 'flex items-center justify-between'}`}>
          <div className="text-sm text-gray-600 text-center">
            Página {currentPage + 1} de {totalPages} • {paginatedData.length} de {filteredCount} modelos
          </div>
          <div className="flex items-center justify-center gap-2">
            <button
