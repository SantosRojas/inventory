...
const EditPumpModal: React.FC<EditBombaModalProps> = ({ isOpen, onClose, onSuccess, bomba }) => {
    ...
    const validateForm = (): boolean => {
        ...
        // Validaciones para todos los users
        ...
        if (isAdmin) {
            if (!formData.qrCode?.trim()) {
                newErrors.qrCode = 'El código QR es requerido';
            }

            if (!formData.institutionId || formData.institutionId === 0) {
            ...
            if (!formData.inventoryDate) {
                newErrors.inventoryDate = 'La fecha de inventario es requerida';
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isAdmin) {
                ...
                updateData.inventoryDate = formData.inventoryDate;
            ...
            await update(bomba.id, updateData);

            onSuccess?.();
            onClose();
        } catch (error) {
            console.error('Error al actualizar bomba:', error);
            const errorMessage = error instanceof Error ? error.message : 'Error desconocido al actualizar la bomba';
