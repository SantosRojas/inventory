...
const InventoryProgressByServiceCharts: React.FC<InventoryProgressByServiceChartsProps> = ({ data, summaryData }) => {
  ...
  const handleServiceDownload = useCallback(async (serviceId: number, serviceName: string, institutionId: number, institutionName: string, pumpsInventoriedThisYear: number, totalPumps: number) => {
    const downloadKey = `${institutionId}-${serviceId}`;
    setDownloadingServiceId(downloadKey);

    try {
      const data = await downloadService.downloadServiceInventory(serviceId, institutionId);
      ...
      const filename = `inventario-servicio-${serviceName.replace(/\s+/g, '-').toLowerCase()}-${institutionName.replace(/\s+/g, '-').toLowerCase()}`;

      const resumenExtra = [
        { Campo: 'Bombas Inventariadas Este Año', Valor: pumpsInventoriedThisYear.toString() },
        { Campo: 'Bombas pendientes por inventariar', Valor: (totalPumps - pumpsInventoriedThisYear).toString() },
        { Campo: 'Total de Bombas', Valor: totalPumps.toString() }
      ];

      // Usar la función de utilidad para generar y descargar el Excel
      downloadInventoryExcel(data, `Reporte de ${serviceName} - ${institutionName}`, filename, resumenExtra);
      ...
      <div className="bg-gray-50 rounded-lg p-2 sm:p-3 min-w-0">
        <div className="flex justify-between items-start mb-2 gap-2">
          ...
          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            ...
            <button
              onClick={() => handleServiceDownload(service.serviceId, service.serviceName, institutionId, institutionName, service.pumpsInventoriedThisYear, service.totalPumps)}
  ...
  const handleDownload = useCallback(async (institution: InstitutionProgressWithTotals, type: 'total' | 'current-year' | 'not-inventoried' | 'overdue-maintenance') => {
    ...
    try {
      ...
      const slug = institutionName.replace(/\s+/g, '-').toLowerCase();

      const downloadMap = {
        'total': {
          fetch: () => downloadService.downloadInventoryTotal(institutionId),
          title: `Reporte de Inventario Total - ${institutionName}`,
          filename: `inventario-total-${slug}`,
          displayName: 'Inventario Total'
        },
        'current-year': {
          fetch: () => downloadService.downloadInventoryCurrentYear(institutionId),
          title: `Reporte de Inventario 2025 - ${institutionName}`,
          filename: `inventario-2025-${slug}`,
          displayName: 'Inventario 2025'
        },
        'not-inventoried': {
          fetch: () => downloadService.downloadInventoryNotInventoried(institutionId),
          title: `Reporte de Pendientes de Inventario - ${institutionName}`,
          filename: `pendientes-inventario-${slug}`,
          displayName: 'Pendientes de Inventario'
        },
        'overdue-maintenance': {
          fetch: () => downloadService.downloadOverdueMaintenance(institutionId),
          title: `Reporte de Mantenimiento Vencido - ${institutionName}`,
          filename: `mantenimiento-vencido-${slug}`,
          displayName: 'Mantenimiento Vencido'
        }
      } as const;

      const config = downloadMap[type];
      if (!config) throw new Error('Tipo de descarga no válido');

      const data = await config.fetch();
      const { title, filename, displayName } = config;

      // Validar si hay datos para descargar
      if (!data || data.length === 0) {
        notifyWarning(
          `Sin datos para ${displayName}`,
          `No hay elementos disponibles para descargar en la categoría "${displayName}" de ${institution.institutionName}.`
        );
        setOpenMenuId(null);
        return;
      }

      // Generar resumen extra
      const { modelCount, serviceCount, inventoriedThisYear } = getInventoryCounts(data);
      ...
      if (type === 'total') {
        resumenExtra.push(
          { Campo: 'Bombas Inventariadas Este Año', Valor: institution.inventoriedItems.toString() },
          { Campo: 'Bombas Pendientes por Inventariar', Valor: (institution.totalItems - institution.inventoriedItems).toString() },
          { Campo: 'Total de Bombas', Valor: institution.totalItems.toString() }
        );
      } else if (type === 'overdue-maintenance') {
        resumenExtra.push(
          { Campo: 'Bombas Inventariadas Este Año', Valor: inventoriedThisYear.toString() },
          { Campo: 'Bombas No Inventariadas Este Año', Valor: (data.length - inventoriedThisYear).toString() },
          { Campo: 'Total de Bombas', Valor: data.length.toString() }
        );
      } else {
        resumenExtra.push({ Campo: 'Total de Bombas', Valor: data.length.toString() });
      }


      // Usar la función de utilidad para generar y descargar el Excel
      downloadInventoryExcel(data, title, filename, resumenExtra);

      // Mostrar notificación de éxito
      notifySuccess(
        'Descarga completada',
        `Se han descargado ${data.length} elementos de ${displayName} para ${institution.institutionName}.`
      );
                      ...
                      <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-3 sm:gap-4">
                        ...
                        <div className="relative download-menu-container">
                          ...
                          {openMenuId === institution.institutionId && (
                            <div className="absolute right-0 mt-2 w-64 sm:w-72 bg-primary text-primary rounded-lg shadow-lg z-10 max-w-[calc(100vw-2rem)]">
                              <div className="py-2">
                                <button
                                  onClick={() => handleDownload(institution, 'total')}
                                ...
                                >
                                  ...
                                  <div className="flex flex-col min-w-0">
                                    ...
                                    <span className="text-xs text-secondary truncate">Todas las bombas</span>
                                  </div>
                                </button>
                                <button
                                  onClick={() => handleDownload(institution, 'current-year')}
                                ...
                                >
                                  ...
                                  <div className="flex flex-col min-w-0">
                                    ...
                                    <span className="text-xs text-secondary truncate">Solo inventariadas este año</span>
                                  </div>
                                </button>
                                <button
                                  onClick={() => handleDownload(institution, 'not-inventoried')}
                                ...
                                >
                                  ...
                                  <div className="flex flex-col min-w-0">
                                    ...
                                    <span className="text-xs text-secondary truncate">Sin inventariar este año</span>
                                  </div>
                                </button>
                                <button
                                  onClick={() => handleDownload(institution, 'overdue-maintenance')}
                                ...
                                >
                                  ...
                                  <div className="flex flex-col min-w-0">
                                    ...
                                    <span className="text-xs text-secondary truncate">Requieren mantenimiento</span>
                                  </div>
                                </button>
                              </div>
                            </div>
